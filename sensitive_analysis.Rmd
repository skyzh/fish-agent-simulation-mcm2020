...
read_data <- function(path) {
  x1 = roll_mean_data(read_from(sprintf("./log_process/result%s_0.1.csv", path)))
  x1$Noise = rep(0.1)
  x2 = roll_mean_data(read_from(sprintf("./log_process/result%s_0.5.csv", path)))
  x2$Noise = rep(0.5)
  x3 = roll_mean_data(read_from(sprintf("./log_process/result%s_1.0.csv", path)))
  x3$Noise = rep(1)
...
x4 = roll_mean_data(read_from("./log_process/result_R_10.csv"))
x4$Noise = rep(0.1)
x4$Type = "Noise"
x5 = roll_mean_data(read_from("./log_process/result_R_5.csv"))
x5$Noise = rep(0.5)
x5$Type = "Noise"
...
ggplot(xx, aes(year, left)) + 
  ...
  theme_minimal()
ggsave('figure/noise_fish_total.pdf', width=6, height=3)
```
...
ggsave('figure/noise_fish_available.pdf', width=6, height=3)
...
ggsave('figure/noise_fish_ratio.pdf', width=6, height=4)
