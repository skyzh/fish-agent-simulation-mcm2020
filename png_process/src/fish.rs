...
pub fn one_epoch(map: &Vec<TemperatureMap>) {
    ...
    for (map_idx, t_map) in map.iter().enumerate() {
        ...
        if OUTPUT_EPOCH_JSON && MAKE_JSON_AFTER(t_map.year, t_map.month) {
        ...
        if OUTPUT_INDEX_IMAGE && MAKE_JSON_AFTER(t_map.year, t_map.month) {
        ...
        if MAKE_JSON_AFTER(t_map.year, t_map.month) {
